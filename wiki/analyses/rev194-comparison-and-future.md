...
alpha0.10 ソースを GitHub に出す前提は妥当。著者本人がオーナーで、[GRINEdit](../entities/GRINEdit.md)ページでも権利上の制約はないと整理済み。公開対象は、一次資料 zip、展開済み alpha0.10 ソース、復元スクリプト、wiki。`build/`, `lib/`, `raw/`, `alpha0.10-src/cachedir/` は生成物または未整理メモとして公開対象から外す。
