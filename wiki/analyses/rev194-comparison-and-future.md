...
現代で Java 版をそのまま発展させる価値は「考古学的復元・仕様抽出」にはあるが、「新規利用されるアプリ」としては低い。SWT/JFace/Jython 2.1/XML-RPC/JYAML/JyConsole など依存が古く、UI 配布も Windows SWT 前提であるため、長期的な価値は JS/Electron または Web/WASM 系への仕様移植で得る方が大きい（[macOS 復元手順](macos-restoration-alpha0.10.md)、[rev194 バイナリ](../sources/grinedit-bin-rev194.md)より）。2026-06-10 時点で、rev194 を現代 macOS 上で起動して挙動観察する試みは中止し、Java/SWT 版の延命は無理と判断した。
...
2026-06-10 の判断として、Java 版は「読むための歴史的実装」に留める。挙動観察がろくにできていない状態で SWT/Jython 依存を現代化するのは費用対効果が悪く、延命はしない。復元ソースから抽出する対象は、データモデル、保存形式、物理法則、操作 API などの仕様に限定する。
...
1. `rev194-recovered-src/` から、移植に必要な最小仕様を抽出する。
2. 対象を Graph model、Renderable vertex/edge、layout/physical law、YAML/JSON import/export、CommonGateway 相当 API に絞る。
3. Java UI の延命ではなく、TypeScript/Web/Electron 実装に寄せる。
