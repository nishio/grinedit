...
unpin 操作は alpha0.10 の `pythonScripts/menu/s_v_disanchored.py` に `make selected vertex dis-anchored` として存在するが、menu 登録行はコメントアウトされている。rev194 では明示 menu item は見つからず、`RenderableVertex.setDisanchored()` と `MO_MoveVertex` の Shift release による anchor remove が残っている。Web 版では操作発見性を優先し、選択中 vertex の pin を外す `Unpin` button を追加する。

`web/` では `CommandGateway.unpinVertex()` と toolbar の `Unpin` button を追加した。button は選択中 vertex が pinned の場合だけ有効になり、実行後は Pinned count と YAML preview から `pinned: true` が消える。
